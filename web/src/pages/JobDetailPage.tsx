...
export function JobDetailPage() {
  ...
  async function analyze() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("job.errorAnalyze"));
    } finally {
      setBusy(null);
    }
  }
  ...
  if (!job) {
    return <p className="label">{error ?? t("job.loading")}</p>;
  }
    ...
    <div className="fade-in max-w-3xl">
      ...
      <div className="mt-5 flex items-start gap-4 border-b border-[var(--ink)] pb-6">
        ...
        <div>
          ...
          >
            {job.salaryRaw ?? t("job.salaryUnknown")}
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <ActionBtn onClick={() => void analyze()} disabled={!!busy}>
          {busy === "analyze" ? t("job.analyzing") : t("job.analyze")}
        </ActionBtn>
        <ActionBtn onClick={() => setLetterOpen(true)} disabled={!!busy} variant="ghost">
          {coverLetter ? t("job.viewLetter") : t("job.letter")}
        </ActionBtn>
        <ActionBtn onClick={() => void addToPipeline()} disabled={!!busy} variant="ghost">
          {job.application ? t("job.inPipeline") : t("job.addPipeline")}
        </ActionBtn>
        {job.url && (
      ...
      {job.analysis && (
        <section className="panel mt-6">
          <p className="label">{t("job.analysisEyebrow")}</p>
          <h2 className="mt-1 text-xl">{t("job.analysisTitle")}</h2>
      ...
      {coverLetter && (
        <section className="panel">
          <p className="label">{t("job.draft")}</p>
          <h2 className="mt-1 text-xl">{t("job.letterTitle")}</h2>
      ...
      <section className="panel">
        <p className="label">{t("job.source")}</p>
        <h2 className="mt-1 text-xl">{t("job.description")}</h2>
